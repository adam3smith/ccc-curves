...
function normalize(sing) {   // use only for non-zero vectors
                  const n  = norm1(sing);
                  const unit = [];
                  if ( n > 0 )
...
const vecSum1 = ((sing1, sing2) => {
			if (!(Array.isArray(sing1)) ) 
				{ const sum = sing1 + sing2; 
				  return sum;}
			else	{
				const sum = [];
            	for (let k=0; k < sing1.length; k++) {
...
function intersectLines(pt1,pt2, qt1,qt2)	{
			const vp	= vecDif1(pt2,pt1); // line: pt1 + t*vp
			const vq	= vecDif1(qt2,qt1); // line: qt1 + s*vq
			const vpp	= [-vp[1], vp[0]];
			const pq1	= vecDif1(pt1,qt1);
			const nn	= dotProd1(vq,vpp);
			let ss;
			const suc	= (abs(nn) > epsD*epsD);
			if (!suc) { ss = 1/epsD/epsD }
			else 	  { ss = dotProd1(pq1, vpp)/nn; }
			const res	= linComb1(1-ss, ss, qt1,qt2);
		return [suc, res];
}	
