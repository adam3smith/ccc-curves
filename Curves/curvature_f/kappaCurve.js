...
svgEl.setAttribute("viewBox"," -30 -140 340 320");
...
let kappaParam = "72"; 			 // string, max = 132
let bPar       = kappaParam/120; // number see function CrvtParam()
...
let scale;  // scaling factor for drawing
let s0;
let s1;
let c0;
let v0;
...
function initialize() {
		scale  	= 60*sqrt(bPar);
		if ((kappaParam > 74) && (kappaParam < 114))
			{scale  	= 100*sqrt(bPar);}
		s0	 	= 0.001;
		s1 		= 32.001; 
		c0 		=[1.2/bPar,2.0-exp(bPar)]; 
		if (kappaParam < 61) 			{c0 =[3.5, 4.5-exp(bPar)];}
		if (kappaParam > 117)   {c0 =[1.1/bPar,4.0-exp(bPar)];}
		v0 		= [1,0];
	if (choose == 1)
	{	scale = 50;
		s0 	= -5-pi;  s1 = 8.1; 
		n 	= 256;
		c0 	=[1.2/bPar,1]; v0 = [1,0];					
	}
		ds 	= (s1 - s0)/n;
}
...
inputKappaParam.value = kappaParam;
function CrvtParam() {
		window.clearInterval(loopID);
		checkboxOsc.checked = false;
		kappaParam = min(132, max(12, inputKappaParam.value));
		kappaParam = floor(kappaParam);
		inputKappaParam.value = kappaParam;
		bPar       = kappaParam/120;  // Automatic conversion
		initialize();
		if (choose == 0)	{
				curve = CurveByCurvature2D(s0,s1,n,inStp, kappa, c0,v0);	}
