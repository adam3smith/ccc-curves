...
function cassini(p) {
		return	((sqr(p[0]-1)+sqr(p[1]))*(sqr(p[0]+1)+sqr(p[1]))-1);
	}
	
function gradCassini(p)	{
			const gtc	= [];
			const fac	= sqr(p[0]) + sqr(p[1]);
				gtc[0]	=  4*p[0]*(fac-1);
				gtc[1]	=  4*p[1]*(fac+1);
		return gtc;
	}
	
function tangCassini(p)	{
			const tgtc	= [];
			const fac	= sqr(p[0]) + sqr(p[1]);
				tgtc[0] = -4*p[1]*(fac+1);
				tgtc[1] =  4*p[0]*(fac-1);
 ...
 function ddLevel(p) {
			const ddfo 	= [];
				ddfo[0]	= [4*(3*sqr(p[0])+sqr(p[1])-1), 8*p[0]*p[1]];
				ddfo[1]	= [8*p[0]*p[1], 4*(sqr(p[0]) + 3*sqr(p[1])+1)];
		return ddfo;	
 }	
...
function fillCurve() {
		...
		for (let i=0; i<n+1; i++)
		{ 
			t1		= tangCassini(start);
			n0		= norm1(t1);
			if (choose < 2)  {ds =  1/n0/96;} 
			else if (!(choose==8))  {ds = 1/n0/32;} 
			else
		    	{	if (n0 < epsD*16) {t1 = [epsD*16, epsD*16]; n0 = epsD*16;}
		    		ds	= 1/max(1/256, n0)/128;}
		curve[i] 	= [start, t1 ];
		step[i]		= ds;
			nextP 	= LevelODEt2(ds/2, start, tangCassini, ddLevel);
			nextP 	= LevelODEt2(ds/2, nextP, tangCassini, ddLevel);
			v1 = cassini(nextP);  // This Newton step improves accuracy
			t1 = gradCassini(nextP);
			nextP = linComb1(1,(v0-v1)/dotProd1(t1,t1)  , nextP,t1);
			//if (i==100){console.log("start, nextP = ",start, nextP,v0,cassini(nextP));}
			start = nextP;
		}
	return curve;
}
