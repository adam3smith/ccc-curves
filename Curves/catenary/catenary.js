...
svgEl.setAttribute("viewBox"," -250 -120 370 320");
...
const n 	= 256;
const RR	= 10000;
let scale 	= 32; // scale factor for drawing
let s0    	= -3.0 - 6*morph;
let s1  	= +3.0;
let ds 		= (s1 - s0)/n;	
...
function theCurve(t) {
			const cvx	= (t+0)*scale;
			const cvy   = (-morph*exp(t) - (1-morph)*cosh(t) + 6)*scale;
		return [cvx, cvy];
	}
function dtheCurve(t) {
		return [ scale, (-morph*exp(t) - (1-morph)*sinh(t))*scale ];
...
const toggleInversion = (() => {
    ...
    if ( checkboxInvert.checked ) {
         svgEl.setAttribute("viewBox"," -200 -120 370 320");
        }
    else {svgEl.setAttribute("viewBox"," -250 -120 370 320"); 
    	}
		fillCurve();
		makeTXTforpath();
		makeNormalsSVG();
...
function getMorphParam() {
		//window.clearInterval(loopID1);
		checkboxOsc.checked = false;
		morph 	= min(1, max(0, inputParamFr.value));
		inputParamFr.value = morph;
		fillCurve();
		makeTXTforpath();
		makeNormalsSVG();
