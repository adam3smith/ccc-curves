...
function theCurve(t) {
			...
			const cvx	= bb*tan(tt) + kk*sin(tt);
			const cvy   = -bb - kk*cos(tt);
		return [cvx, cvy];
	}
...
function makeConstructSvg(j)	{
	...
	const nq	= sqrt(sqr(qx) + sqr(qy));
	const mx 	= 0;
	const my 	= 0;
	let stx		= scale*4*Mx/nM;
	let sty		= scale*4*My/nM;
	
		//const tan	= dtheCurve(tj);
	const ans	= intersectLines([mx,my],[-My,Mx], [Mx,My],[Mx,0]);
	const bx 	= ans[1][0];
	const by 	= ans[1][1];
	let tx		= 2*(qy - by);
	let ty		= 2*(bx - qx);
    ...
    txtst = 'M'+mx+' '+my+'L'+Mx+' '+My+'L'+qx+' '+qy;
    rstick.setAttribute("d", txtst );
    
    laxis.setAttribute("x1", -100*Mx);
	laxis.setAttribute("y1", -100*My);
	laxis.setAttribute("x2", 100*Mx);
	laxis.setAttribute("y2", 100*My);
